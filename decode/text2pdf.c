...
22 Jan 2015 Added vertical and horizontal margin options. Vince Risi
**************************************************************************/
...
long StartPage(){
  ...
  sprintf(buf, "1 0 0 1 %d %d Tm\n", horzMargin, pageHeight - vertMargin); writestr(buf);
...
void WritePages(){
  ...
  while (!atEOF) {
	...
	if (atBOP) {
      ...
      if (column <= columns) {
	sprintf(buf, "1 0 0 1 %d %d Tm\n",
		(pageWidth / 2) + horzMargin, pageHeight - vertMargin);
	writestr(buf);
      }
    }
    EndPage(beginstream);
...
void ShowHelp(){
  ...
  printf("  -V<height>\tvertical margin (default=%d)\n", vertMargin);
  printf("  -H<width>\thorizontal margin (default=%d)\n", horzMargin);
...
int main(int argc, char **argv){
  ...
  while (i < argc) {
	...
	break;
      case 'V':
	vertMargin = atoi(++argv[i]);
	if (vertMargin < 0) vertMargin = 0;
	break;
      case 'H':
	horzMargin = atoi(++argv[i]);
	if (horzMargin < 0) horzMargin = 0;
	break;
      case 'l':
	lines = atoi(++argv[i]);
	if (lines < 1) lines = 1;
