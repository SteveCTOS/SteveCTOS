      ...
      *
       ...
       01  HEAD1.
           ...
           "INVOICE REGISTER BY ACCOUNT NUMBER - L/YR".
       ...
       01  HEAD4.
           ...
           03  FILLER           PIC X(21) VALUE "  AMOUNT      AMOUNT".
       ...
       01  TOTAL-LINE.
           ...
           03  FILLER           PIC X(9) VALUE "TOTALS:".
      ...
      *
       ...
       CONTROL-000.
           ...
           MOVE 320 TO POS
           DISPLAY "** L/YR Invoice Register By Account Report **"
            AT POS
           MOVE 420 TO POS
           DISPLAY "*********************************************"
            AT POS.
      ...
      *
       ...
       PR-001.
           ...
           MOVE 2310 TO POS
           DISPLAY "READING L/YR INVOICE REGISTER BY NUMBER..." AT POS.
           MOVE WS-BEG-DATE TO INCR-LY-DATE
       ...
       PR-003.
           ...
           IF WS-INCR-LY-ST1 = 10
              GO TO PR-999.
           IF WS-INCR-LY-ST1 NOT = 0
             MOVE "RECORD LOCKED AT ANOTHER STATION, <CANCEL> TO RETRY."
             TO WS-MESSAGE
             PERFORM ERROR1-000
             ...
             PERFORM ERROR-020
             GO TO PR-003.
           IF INCR-LY-DATE < WS-BEG-DATE
               GO TO PR-003.
           IF INCR-LY-DATE > WS-END-DATE
               GO TO PR-999.
             
           MOVE INCR-LY-INVOICE TO D-INVNO.
           
           IF INCR-LY-ACCOUNT NOT = WS-ACCOUNT-NUMBER
               GO TO PR-003.
       PR-010.
           IF WS-LINE > 60
               PERFORM PRINT-HEADINGS.
           MOVE INCR-LY-INVOICE              TO D-INVNO
           MOVE WS-TYPE-DESC (INCR-LY-TRANS) TO D-TYPE
           MOVE INCR-LY-DATE                 TO SPLIT-DATE
           PERFORM CONVERT-DATE-FORMAT
           MOVE DISPLAY-DATE         TO D-DATE
           MOVE INCR-LY-SB-TYPE      TO D-SOLDBY
           MOVE INCR-LY-ACCOUNT      TO D-CUSTNO
           MOVE INCR-LY-NAME         TO D-NAME
           MOVE INCR-LY-SALES        TO D-SALES
           MOVE INCR-LY-INVCRED-AMT  TO D-INVAMT
           MOVE INCR-LY-TAX          TO D-TAX
           MOVE INCR-LY-ADDONS       TO D-ADDON
           MOVE INCR-LY-DISCOUNT     TO D-DISCOUNT
           MOVE INCR-LY-INVCRED-COST TO D-COST.
       PR-020.
           IF INCR-LY-TRANS = 1
              COMPUTE WS-IMARGIN = INCR-LY-INVCRED-AMT -
                  (INCR-LY-INVCRED-COST + INCR-LY-TAX + INCR-LY-ADDONS)
              MOVE WS-IMARGIN          TO D-MARGIN
              COMPUTE WS-IPERC =
                  (WS-IMARGIN / INCR-LY-INVCRED-COST) * 100
              MOVE WS-IPERC            TO D-PERC
              ADD INCR-LY-INVCRED-AMT  TO WS-TOTI-INVOICE
              ADD INCR-LY-TAX          TO WS-TOTI-TAX
              ADD INCR-LY-ADDONS       TO WS-TOTI-ADDON
              ADD INCR-LY-DISCOUNT     TO WS-TOTI-DISCOUNT
              ADD INCR-LY-INVCRED-COST TO WS-TOTI-COST
              ADD 1                    TO WS-INVNO
              GO TO PR-900.
           IF INCR-LY-TRANS = 6
              COMPUTE WS-IMARGIN = INCR-LY-INVCRED-AMT -
              ...
              COMPUTE WS-CPERC =
                   (WS-CMARGIN / INCR-LY-INVCRED-COST) * 100
              MOVE WS-CPERC            TO D-PERC
              ADD INCR-LY-INVCRED-AMT  TO WS-TOTC-INVOICE
              ADD INCR-LY-TAX          TO WS-TOTC-TAX
              ADD INCR-LY-ADDONS       TO WS-TOTC-ADDON
              ADD INCR-LY-DISCOUNT     TO WS-TOTC-DISCOUNT
              ADD INCR-LY-INVCRED-COST TO WS-TOTC-COST
              ADD 1                    TO WS-CRNO.
       PR-900.
           IF WS-PERC-ACCEPT NOT = " "
            IF INCR-LY-TRANS = 1
             IF WS-PERC < WS-IPERC
                ...
                SUBTRACT INCR-LY-INVCRED-AMT  FROM WS-TOTI-INVOICE
                SUBTRACT INCR-LY-TAX          FROM WS-TOTI-TAX
                SUBTRACT INCR-LY-ADDONS       FROM WS-TOTI-ADDON
                SUBTRACT INCR-LY-DISCOUNT     FROM WS-TOTI-DISCOUNT
                SUBTRACT INCR-LY-INVCRED-COST FROM WS-TOTI-COST
                GO TO PR-003.
           IF WS-PERC-ACCEPT NOT = " "
            IF INCR-LY-TRANS = 6
                SUBTRACT 1                    FROM WS-CRNO
                SUBTRACT INCR-LY-INVCRED-AMT  FROM WS-TOTC-INVOICE
                SUBTRACT INCR-LY-TAX          FROM WS-TOTC-TAX
                SUBTRACT INCR-LY-ADDONS       FROM WS-TOTC-ADDON
                SUBTRACT INCR-LY-DISCOUNT     FROM WS-TOTC-DISCOUNT
                SUBTRACT INCR-LY-INVCRED-COST FROM WS-TOTC-COST
                GO TO PR-003.
           IF WS-RAND-ACCEPT NOT = " "
            IF INCR-LY-TRANS = 1
             IF WS-RAND > INCR-LY-INVCRED-AMT
                SUBTRACT 1                    FROM WS-INVNO
                SUBTRACT INCR-LY-INVCRED-AMT  FROM WS-TOTI-INVOICE
                SUBTRACT INCR-LY-TAX          FROM WS-TOTI-TAX
                SUBTRACT INCR-LY-ADDONS       FROM WS-TOTI-ADDON
       ...
       PR-950.
           IF INCR-LY-SALES = 6 OR = 52
             GO TO PR-960
           ELSE
             GO TO PR-003.
       PR-960.
           IF INCR-LY-TRANS = 1
              ADD INCR-LY-INVCRED-AMT  TO WS-XPORTI-INVOICE
              ADD INCR-LY-TAX          TO WS-XPORTI-TAX
              ADD INCR-LY-ADDONS       TO WS-XPORTI-ADDON
              ADD INCR-LY-DISCOUNT     TO WS-XPORTI-DISCOUNT
              ADD INCR-LY-INVCRED-COST TO WS-XPORTI-COST
              ADD 1                 TO WS-XPORT-INVNO.
           IF INCR-LY-TRANS = 6
              ADD INCR-LY-INVCRED-AMT  TO WS-XPORTC-INVOICE
              ADD INCR-LY-TAX          TO WS-XPORTC-TAX
              ADD INCR-LY-ADDONS       TO WS-XPORTC-ADDON
              ADD INCR-LY-DISCOUNT     TO WS-XPORTC-DISCOUNT
              ADD INCR-LY-INVCRED-COST TO WS-XPORTC-COST
      ...
      *
       ...
       OPEN-014.
           ...
           IF WS-INCR-LY-ST1 NOT = 0
              ...
              MOVE "L/YR REGISTER FILE BUSY ON OPEN, 'ESC' TO RETRY."
              TO WS-MESSAGE
