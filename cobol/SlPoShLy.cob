      ...
      *
       ...
       01  HEAD1.
           ...
           "** SHORT PURCHASE ORDERS INQUIRY - L/YR **".
      ...
      *
       ...
       CONTROL-000.
           ...
           MOVE 0310 TO POS
           DISPLAY "** SHORT PURCHASE ORDER INQUIRY BY INVOICE **"
           AT POS
           MOVE 0410 TO POS
           DISPLAY "*********************************************"
           AT POS.
      ...
      *
       ...
       RDALL-005.
           ...
           START INCR-LY-REGISTER KEY NOT < INCR-LY-PORDER
                INVALID KEY NEXT SENTENCE.
           IF WS-INCR-LY-ST1 NOT = 0
               MOVE "REGISTER-LY BUSY ON START, 'ESC' TO EXIT."
               TO WS-MESSAGE
               PERFORM ERROR1-000
               MOVE WS-INCR-LY-ST1 TO WS-MESSAGE
               PERFORM ERROR-MESSAGE
       ...
       RDALL-010.
            ...
            IF WS-INCR-LY-ST1 = 10 OR = 91
               ...
               GO TO RDALL-900.
            IF WS-INCR-LY-ST1 NOT = 0
             MOVE "REGISTER-LY BUSY READ-NEXT, IN 1 SEC GOING TO RETRY."
               TO WS-MESSAGE
               PERFORM ERROR1-000
               MOVE WS-INCR-LY-ST1 TO WS-MESSAGE
               PERFORM ERROR-000
           ...
           IF WS-KEY = "O"
            IF INCR-LY-TRANS NOT = 4 AND NOT = 7
               ...
               GO TO RDALL-010.
           IF WS-KEY = "R"
            IF INCR-LY-TRANS NOT = 3
               MOVE 2910 TO POS
               DISPLAY "Be Patient, reading next Repair........" AT POS
               GO TO RDALL-010.
           IF WS-KEY = "R"
            IF INCR-LY-PRINTED NOT = "R"
               MOVE 2910 TO POS
               DISPLAY "Be Patient, reading next Repair........" AT POS
               GO TO RDALL-010.
           IF WS-KEY = "Y"
       ...
       RDALL-020.
           ...
           MOVE INCR-LY-TRANS      TO WS-INCR-TRANS (SUB-1)
           MOVE INCR-LY-INVOICE    TO WS-INCR-INVOICE (SUB-1).
      ...
      *
       ...
       RDONLY-005.
           ...
           MOVE WS-INCR-TRANS (SUB-1)     TO INCR-LY-TRANS
           MOVE WS-INCR-INVOICE (SUB-1)   TO INCR-LY-INVOICE
       ...
       RDONLY-010.
           ...
           IF WS-INCR-LY-ST1 = 10
               ...
               GO TO RDONLY-999.
           IF WS-INCR-LY-ST1 NOT = 0
             MOVE "REGISTER-LY BUSY READ-ONLY, IN 1 SEC GOING TO RETRY."
               TO WS-MESSAGE
               PERFORM ERROR1-000
               MOVE WS-INCR-LY-ST1 TO WS-MESSAGE
               PERFORM ERROR-000
      ...
      *
       ...
       CH-O-T-010.
           IF WS-NO-CH = 1
              MOVE INCR-LY-PORDER TO W-PO-CH1
              GO TO CH-O-T-100.
           IF WS-NO-CH = 2
              MOVE INCR-LY-PORDER TO W-PO-CH2
              GO TO CH-O-T-200.
           IF WS-NO-CH = 3
              MOVE INCR-LY-PORDER TO W-PO-CH3
              GO TO CH-O-T-300.
           IF WS-NO-CH = 4
              MOVE INCR-LY-PORDER TO W-PO-CH4
              GO TO CH-O-T-400.
           IF WS-NO-CH = 5
              MOVE INCR-LY-PORDER TO W-PO-CH5
              GO TO CH-O-T-500.
       CH-O-T-100.
      ...
      *     PERFORM CHECK-ORDER-TYPE.
        ...
        PRR-020.
           ...
           MOVE INCR-LY-INVOICE      TO D-1STINV
           ...
           IF WS-KEY = "O"
            IF INCR-LY-TRANS = 4
             IF INCR-LY-PRINTED = "N"
               MOVE " **PART ORDER**           " TO D-COMPLETE.
           IF WS-KEY = "O"
            IF INCR-LY-TRANS = 4
             IF INCR-LY-PRINTED = "S"
