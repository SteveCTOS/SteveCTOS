      ...
      *
       ...
       77  WS-COMPUTE-AMT       PIC S9(8)V99 VALUE 0.
       ...
       01  WS-TRANS-DESCRED REDEFINES WS-TRANS-TYPE.
           03  WS-TRANS-DESC   PIC X(7) OCCURS 27.
       01  WS-CBNUMBER.
           03  WS-HEAD-SUB.
       ...
       01  HEAD1.
           ...
           "** CASHBOOK / BANK STATEMENT REPORT **".
      ...
      *
       ...
       CONTROL-000.
           ...
           MOVE 315 TO POS
           DISPLAY "** CASHBOOK / BANK STATEMENT REPORT **" AT POS
           MOVE 415 TO POS
           DISPLAY "**************************************" AT POS.
       CONTROL-003.
           Copy "PrinterAccept".
      ...
      *
       ...
       GET-015.
           MOVE 1510 TO POS
           DISPLAY "Enter D=Debtor, C=Creditors, S=G/Ledger, Blank=ALL."
                AT POS
           ADD 50 TO POS
           DISPLAY " : [ ]" AT POS
           ADD 4 TO POS.
       ...
       GET-020.
            MOVE "Y" TO WS-RANGE4.
            MOVE 1810 TO POS
            DISPLAY "Do you wish to print ALL Outstanding Transactions"
                AT POS
            ADD 51 TO POS
            DISPLAY ": [ ]" AT POS
            ADD 3 TO POS
           ...
           IF WS-PER > 0
            IF CBTRANS-NO NOT = WS-PER
             IF CBTRANS-ALLOCATED NOT = "N" AND NOT = "H"
               GO TO PRR-002.
           IF WS-RANGE3 = "S"
            IF CBTRANS-TYPE-OF-POST = "G" OR = "S"
               GO TO PRR-008.
           IF WS-RANGE3 NOT = " "
            IF CBTRANS-TYPE-OF-POST NOT = WS-RANGE3
               GO TO PRR-002.
       PRR-008.
           IF WS-RANGE4 = "N"
            IF WS-PER NOT = 0
       ...
       PRR-020.
           ...
           IF WS-PER = CBTRANS-NO
            IF CBTRANS-ALLOCATED = "Y"
             IF CBTRANS-DATE < GL-BEGDATE (SUB-1)
                GO TO PRR-002.
           IF WS-PER = CBTRANS-NO
            IF CBTRANS-ALLOCATED = "Y"
             IF CBTRANS-DATE > GL-ENDDATE (SUB-1)
                GO TO PRR-002.
      ...
      *
       ...
       END-100.
            ...
            MOVE "DEBTORS DEBIT-AMT    :" TO TOT-DESC
            MOVE WS-DR-AMT-DR             TO TOT-AMOUNT
            WRITE PRINT-REC FROM TOTAL-LINE
            MOVE "DEBTORS CREDIT-AMT   :" TO TOT-DESC
            MOVE WS-DR-AMT-CR             TO TOT-AMOUNT
            WRITE PRINT-REC FROM TOTAL-LINE.

            MOVE " " TO PRINT-REC.
            WRITE PRINT-REC.
            MOVE "CREDITOR DEBIT-AMT   :" TO TOT-DESC
            MOVE WS-CR-AMT-DR             TO TOT-AMOUNT
            WRITE PRINT-REC FROM TOTAL-LINE
            MOVE "CREDITOR CREDIT-AMT  :" TO TOT-DESC
            MOVE WS-CR-AMT-CR             TO TOT-AMOUNT
            WRITE PRINT-REC FROM TOTAL-LINE.
            ...
            MOVE " " TO PRINT-REC.
            WRITE PRINT-REC.
            COMPUTE WS-COMPUTE-AMT = CB-BALANCE -
                 (WS-OS-AMT-DR + WS-OS-AMT-CR).
            MOVE "BAL AS PER BANK STATE:" TO TOT-DESC
            MOVE WS-COMPUTE-AMT           TO TOT-AMOUNT
            WRITE PRINT-REC FROM TOTAL-LINE.
