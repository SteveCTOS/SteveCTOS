      ...
      *
       ...
       GET-000.
        ...
        GET-001.              
            ...
            MOVE F-NAMEFIELD TO GLTRANS-REFERENCE.
            IF F-EXIT-CH = X"0C"
                 MOVE WS-REFERENCE TO GLTRANS-REFERENCE
                 MOVE WS-TRANSNO   TO GLTRANS-TRANS
                 MOVE WS-TYPE      TO GLTRANS-TYPE
                 PERFORM START-TRANS
            ...
            IF F-EXIT-CH = X"07"
                 ...
                 PERFORM DISPLAY-FORM
                 GO TO GET-001.
            IF GLTRANS-REFERENCE NOT > SPACES
                 MOVE "PLEASE ENTER A REFERENCE NUMBER"
                 TO WS-MESSAGE
        ...
        GET-002.
            ...
            MOVE NUMERIC-RATE TO GLTRANS-TRANS F-EDNAMEFIELDNUM.
        ...
        GET-003.              
            ...
            MOVE F-NAMEFIELD TO ALPHA-RATE.
            PERFORM DECIMALISE-RATE.
            MOVE NUMERIC-RATE TO GLTRANS-TYPE.
            IF F-EXIT-CH = X"01"
                 GO TO GET-002.
            IF F-EXIT-CH = X"0C"
                 MOVE WS-TYPE    TO GLTRANS-TYPE
        ...
        GET-004.
            ...
            MOVE "TRANSNO" TO F-FIELDNAME.
            MOVE 7 TO F-CBFIELDNAME.
            MOVE GLTRANS-TRANS TO F-EDNAMEFIELDNUM.
            MOVE 6 TO F-CBFIELDLENGTH.
            PERFORM WRITE-FIELD-NUMERIC.

            MOVE "TYPE" TO F-FIELDNAME.
            MOVE 4 TO F-CBFIELDNAME.
            MOVE GLTRANS-TYPE TO F-EDNAMEFIELDANAL.
            MOVE 2 TO F-CBFIELDLENGTH.
        ...
        GET-005.
            ...
            MOVE GLTRANS-ACCOUNT-NUMBER TO F-NAMEFIELD.
      ...
      *
       ...
       FILL-005.
            ...
            MOVE 3 TO F-CBFIELDLENGTH.
            PERFORM READ-FIELD-ALPHA.
            MOVE F-NAMEFIELD TO GLTRANS-PERIOD.
            IF F-EXIT-CH = X"07"
               PERFORM CLEAR-FORM
               GO TO FILL-999.
            IF F-EXIT-CH = X"01"
       ...
       FILL-010.
            ...
            MOVE SPLIT-DATE TO GLTRANS-DATE.
       ...
       FILL-020.
            ...
            MOVE 25 TO F-CBFIELDLENGTH.
            PERFORM READ-FIELD-ALPHA.
            MOVE F-NAMEFIELD TO GLTRANS-LINE-DESC.
            IF F-EXIT-CH = X"07"
               PERFORM CLEAR-FORM
               GO TO FILL-999.
            IF F-EXIT-CH = X"0C"
      ...
      *
       ...
       DO-010.
            ...
            IF WS-GLTRANS-ST1 NOT = 0
              MOVE "GL-TRANS BUSY ON DELETE, 'ESC' TO RETRY."
              TO WS-MESSAGE
              PERFORM ERROR1-000
      ...
      *
       CLEAR-FORM SECTION.
       CLSC-000.
             MOVE " " TO GLTRANS-REFERENCE
                         GLTRANS-FUTURE
                         GLTRANS-ACCOUNT-NUMBER
                         GLTRANS-LINE-DESC.
             MOVE 0   TO GLTRANS-TRANS
                         GLTRANS-TYPE
                         GLTRANS-NO
                         GLTRANS-DATE
                         GLTRANS-AMOUNT.
      ...
      *
       ...
       ROR-010.
            ...
            REWRITE GLTRANS-REC
                INVALID KEY NEXT SENTENCE.
            IF WS-GLTRANS-ST1 NOT = 0
              MOVE "GLTRANS BUSY ON REWRITE, 'ESC' TO RETRY."
              TO WS-MESSAGE
              PERFORM ERROR1-000
              MOVE WS-GLTRANS-ST1 TO WS-MESSAGE
              PERFORM ERROR-MESSAGE
       ...
       ROR-020.
            WRITE GLTRANS-REC
                INVALID KEY NEXT SENTENCE.
            IF WS-GLTRANS-ST1 NOT = 0
              MOVE "GLTRANS BUSY ON WRITE, 'ESC' TO RETRY."
              TO WS-MESSAGE
              PERFORM ERROR1-000
              MOVE WS-GLTRANS-ST1 TO WS-MESSAGE
              PERFORM ERROR-MESSAGE
      ...
      *
       ...
       RO-000.
           MOVE 0                 TO WS-GLTRANS-ST1.
           MOVE GLTRANS-REFERENCE TO WS-REFERENCE.
           MOVE GLTRANS-TRANS     TO WS-TRANSNO.
           MOVE GLTRANS-TYPE      TO WS-TYPE.
           START GLTRANS-FILE KEY NOT < GLTRANS-KEY.
       RO-010.
           READ GLTRANS-FILE WITH LOCK
                 INVALID KEY NEXT SENTENCE.
           IF WS-GLTRANS-ST1 = 23 OR 35 OR 49
                ...
                GO TO RO-999.
           IF WS-GLTRANS-ST1 NOT = 0
              MOVE "GLTRANS BUSY ON READ-LOCK, 'ESC' TO RETRY."
              TO WS-MESSAGE
              PERFORM ERROR1-000
              MOVE WS-GLTRANS-ST1 TO WS-MESSAGE
              PERFORM ERROR-MESSAGE
     ...
     *
       ...
       RONX-005. 
           READ GLTRANS-FILE NEXT WITH LOCK
            AT END
              ...
              GO TO RONX-999.
           IF WS-GLTRANS-ST1 NOT = 0
              MOVE "GLTRANS BUSY ON READ-NEXT, 'ESC' TO RETRY."
              TO WS-MESSAGE
              PERFORM ERROR1-000
              MOVE WS-GLTRANS-ST1 TO WS-MESSAGE
              PERFORM ERROR-MESSAGE
      ...
      *
       ...
       RTPRE-001.
           MOVE 0 TO WS-GLTRANS-ST1.
       RTPRE-005. 
           READ GLTRANS-FILE PREVIOUS WITH LOCK
            AT END
              MOVE " " TO GLTRANS-REFERENCE
                          WS-REFERENCE
              MOVE 0   TO GLTRANS-TRANS
            ...
            MOVE "END OF PREVIOUS PAGE SEQUENCE, ENTER A NEW IDENTIFIER"
              ...
              GO TO RTPRE-999.
           IF WS-GLTRANS-ST1 NOT = 0
              MOVE "GLTRANS BUSY ON READ-PREVIOUS, 'ESC' TO RETRY."
              TO WS-MESSAGE
              PERFORM ERROR1-000
              MOVE WS-GLTRANS-ST1 TO WS-MESSAGE
              PERFORM ERROR-MESSAGE
      ...
      *
       ...
       OPEN-000.
            ...
            IF WS-GLTRANS-ST1 NOT = 0
              MOVE "GL-TRANS FILE BUSY ON OPEN, 'ESC' TO RETRY."
              TO WS-MESSAGE
              PERFORM ERROR1-000
              MOVE WS-GLTRANS-ST1 TO WS-MESSAGE
              PERFORM ERROR-MESSAGE
