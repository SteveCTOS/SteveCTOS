#!/usr/bin/env python
# The renaming of Payslyp 'CAMS' Files to 'Cams' & add in the month & year for filing
import glob, os, sys
...
def rename (comp, month):
  for file in glob.glob('/media/ctools/RMB/CAMS%s*' % (comp)):
    file2 = file.replace('CAMS%s' % (comp), '%sCams%s' % (month, comp))
