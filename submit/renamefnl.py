#!/usr/bin/env python
# The renaming of Payslyp 'FUN' Files to 'Fnl' & add in the month & year for filing
import glob, os, sys
...
def rename (comp, month):
  for file in glob.glob('/ctools/temp/%sFUN*' % (comp)):
    file2 = file.replace('%sFUN' % (comp), '%sFnl%s' % (month, comp))
