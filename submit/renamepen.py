#!/usr/bin/env python
# The renaming of Payslyp 'PEN' Files to 'Pen' & add in the month & year for filing
import glob, os, sys
...
def rename (comp, month):
  for file in glob.glob('/ctools/temp/%sPEN*' % (comp)):
    file2 = file.replace('%sPEN' % (comp), '%sPen%s' % (month, comp))
